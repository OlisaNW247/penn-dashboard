    ...
    private func review(_ candidate: SyllabusCandidate, _ scheme: SyllabusGradingScheme) -> some View {
        Form {
            ...
            if scheme.mentionsCurve {
                Section {
                    Text("this syllabus mentions a curve or instructor discretion. smooth can\u{2019}t model that. treat the letter grades as approximate.")
                        .font(.lhfSans(11))
                        .foregroundStyle(Color.v2RingSub)
                }
        ...
        if let syncedSyllabusText,
           let scheme = SyllabusParser.parse(syncedSyllabusText) {
            let candidate = SyllabusCandidate(
                ...
                name: "the syllabus smooth already synced",
